...
class ArticuloRepository @Inject constructor(
    ...
    ) : RepositoryBase<Articulo, ArticuloEntityI>(articuloDao){
...
//    }
    ...
    /* ARTICULO */
    private fun Articulo.toEntity() = ArticuloEntityI(
        id = id,
    ...
    override suspend fun toEntity(domain: Articulo): ArticuloEntityI {
        return domain.toEntity()
    }

    override suspend fun toDomain(entity: ArticuloEntityI): Articulo {
        //Tratamos de obtener la categoria, sino lo conseguimos obtenemos la categoria basica
